        ...
        if bout != None:
            batresp = show_message(
                ...
                + f'\nYou can still use wand by clicking "Yes",\nthis will keep wand open in the backround\nIf you want to close Wand click "No"\nWand will automaticly close in {
                    bout} seconds, if nothing is done',
                "BAT Warning",
                bout,
                True,
            )
            log(
                f"The user selected {
                    batresp} after being asked to wait longer for Wand"
            )
    ...
    if last_version:
        try:
            if float(last_version) < float(script_version):
                log(
                    f"Config on version {
                        last_version} updating to {script_version}"
                )
            elif float(last_version) > float(script_version):
                log(
                    f"Warning: config on version {
                        last_version}; downgrading to {script_version}"
                )
        except Exception as e:
            log(
                f"Warning: config error '{
                    e}'; changing version to {script_version}"
            )
    else:
